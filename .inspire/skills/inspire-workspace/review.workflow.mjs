...
const moduleResults = await parallel(modules.map((m) => () =>
  agent(
    ...
    `Return: module="${m}", reviewed=true ONLY if you actually opened and inspected the module's features/UISpec/prototype/specs files (report files_read = how many you opened), the extracted feature_ids and entities (reused downstream), and findings[] (each with severity, description, file, line, fix_skill).`,
...
const report = await agent(
  ...
  `- Phase 5 Prototype component adoption: corpus-wide counts (a component from .inspire_kb/05_ui/components/ is "adopted" by counting its usages across ALL pages of the horizontal prototype at /prototype — never per single module).\n` +
