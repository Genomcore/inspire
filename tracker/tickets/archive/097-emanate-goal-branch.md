...
- [x] § The branch scheme replaces the per-run turn branch with a **goal branch**
...
- [x] **t=0:** the goal branch is cut from the launch branch when it does not exist and
...
- [x] The goal worktree `.claude/worktrees/emanate-<goal-slug>` **persists while the
...
- [x] Per-unit integration branches are `emanate/<goal-slug>-<unit-slug>-<run-stamp>`,
...
- [x] **Every tool that reads tests names its working directory** in doctrine: `plan` in
      the goal worktree, `results` and `gate` in the unit's verify worktree. § gate and
      § verify say so.
- [x] The run log is committed on the goal branch at each block it writes, so it travels
...
- [x] `materialize.sh`'s seeded `.gitignore` block adds `.claude/worktrees/`; a
...
- [x] `unattended.md` § The morning after and § A/B are rewritten for the goal branch;
      ...
      branch per checkout.
- [x] An upgrade test or golden shows that a second run toward the same goal reports the
...
**Closed 2026-09-14, all nine criteria met, in 0.9.8.** The branch scheme, t=0's two
refusals, the persistent goal worktree, the per-unit branch with the run stamp, the working
directory named at `plan`, `verify` and `gate`, and the log committed on the goal branch are
`run.md`; § The morning after and § A/B are `unattended.md`. `materialize.sh` seeds
`.claude/worktrees/` into the `.gitignore` block and extends an existing block that lacks
the line, which `materialize/01-init-current-tree.sh` asserts on both init and update. The
ninth is the golden `emanate-plan/goal-second-run-reuses-realized`: a second run toward one
goal reads the first's unit as realized and the floor drops from 3 to 1.

**The phase-worktree path this settles** is
`.claude/worktrees/emanate-<goal-slug>-<unit-slug>-<run-stamp>-<phase>` — one rule for all
five phase worktrees, the run stamp in the path.
