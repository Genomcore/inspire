...
# `tracker/tickets/`, not inline in the spec.
#
...
#   .claude/bin/no-todos.sh spec/sdd/auth    # scoped scan
...
scan_file() {
  ...
  while IFS=$'\t' read -r ln marker; do
    ...
    sdd_finding "error" "no-todos" "$file" \
      "body contains $marker marker at line $ln (files state present truth only — move outstanding work to tracker/tickets/)"
    sdd_count_error
  done <<< "$hits"
}
