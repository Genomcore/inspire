...
- `spec/sdd/{module}/{entity}/{module}.{entity}.md` — surgical rewrite of the `## Fields` table (one row per field, deduped across actions, types reconciled) and the `## Touched by` table (one row per action touching this entity, with the touch verb and a short note). Other sections are left exactly as the operator wrote them.
