...
## Purpose
Resolve a registered action's manifest by id. The [[pdd-platform-action-catalog|action catalog]] is the runtime source of truth for which capabilities the platform exposes, as defined in [[adr-plt-06-action-catalog]]; this verb is its read entry point.
...
- **The `id:` field is singular** (`platform::action::resolve`) even though the PDD writes it plural. Both are correct — they're the same action expressed at different layers. The folder layout follows the SDD id: `spec/sdd/platform/action/platform.action.resolve.md`.
...
- **The back-source wikilinks** in `## Purpose` and `## Behavior` tie this descriptor to both the PDD's action-catalog row and the governing ADR — woven into the prose that makes the claim, not appended as trailing references. An operator reading this descriptor doesn't need to know about the plural→singular shift; only that the wikilinks resolve.
