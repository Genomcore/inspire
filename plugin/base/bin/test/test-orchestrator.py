#!/usr/bin/env python3
"""Unit tests for `emanate-orchestrator.py`'s pure logic.
...
SOURCE = os.path.join(os.path.dirname(HERE), "emanate-orchestrator.py")
_spec = importlib.util.spec_from_file_location("emanate_orchestrator", SOURCE)
...
class Slugs(unittest.TestCase):
    ...
    def test_runs_of_foreign_characters_collapse_to_one_hyphen(self):
        self.assertEqual(orchestrator.slugify("auth.user.list.."), "auth-user-list")
        self.assertEqual(orchestrator.slugify("inspire_kb/04_domain/auth"),
                         "inspire-kb-04-domain-auth")
        self.assertEqual(orchestrator.slugify("Users::List"), "users-list")
...
class Citations(unittest.TestCase):
    ...
    def test_a_matching_citation_is_clean(self):
        ...
        findings = orchestrator.classify_citations(
    ...
    def test_ci_04_fires_on_an_uncited_test_claim_and_never_on_a_store_one(self):
        ...
        findings = orchestrator.classify_citations(model, [])
        ...
        with tempfile.TemporaryDirectory() as root:
            ...
            with open(path, "w") as stream:
                stream.write("// @claim auth.user/f/id/nonnull sha256:abcd12\n"
                             ...
                             "it('two', () => {})\n")
            found = orchestrator.scan_citations(["tests"], root)
...
class GateRouting(unittest.TestCase):
    ...
    def test_a_pass_is_a_pass(self):
        self.assertEqual(orchestrator.route_gate_verdict(self.verdict("pass", [])),
                         ("pass", None))

    def test_a_contract_class_stalls_rather_than_reworks(self):
        self.assertEqual(orchestrator.route_gate_verdict(self.verdict("fail", ["GV-00"])),
                         ("stall", "GV-00"))
        self.assertEqual(orchestrator.route_gate_verdict(self.verdict("fail", ["GV-06"])),
                         ("stall", "GV-06"))

    def test_a_failed_citing_test_goes_to_arbitration(self):
        self.assertEqual(orchestrator.route_gate_verdict(self.verdict("fail", ["GV-03"])),
                         ("arbitrate", None))

    def test_a_stall_class_wins_over_arbitration(self):
        self.assertEqual(
            orchestrator.route_gate_verdict(self.verdict("fail", ["GV-03", "GV-06"])),
            ("stall", "GV-06"))

    def test_tester_shaped_classes_route_back_to_the_tester(self):
        for cls in ("GV-01", "GV-02", "GV-04"):
            self.assertEqual(orchestrator.route_gate_verdict(self.verdict("fail", [cls])),
                             ("rework", "tester"))

    def test_everything_else_is_the_implementer_s(self):
        self.assertEqual(orchestrator.route_gate_verdict(self.verdict("fail", ["GV-05"])),
                         ("rework", "implementer"))

    def test_the_digest_reads_the_summary(self):
        self.assertEqual(orchestrator.gate_digest(self.verdict("pass", [])), "pass 3/4")

    def test_the_findings_carry_the_gate_s_own_grammar(self):
        findings = orchestrator.gate_findings(self.verdict("fail", ["GV-05"]))
...
class ShellTools(unittest.TestCase):
    ...
    def test_a_tools_line_is_read_off_the_frontmatter(self):
        self.assertEqual(orchestrator.parse_tools_line(self.shell("tools: Read, Grep, Glob")),
                         ["Read", "Grep", "Glob"])

    def test_a_bracketed_list_reads_the_same(self):
        self.assertEqual(orchestrator.parse_tools_line(self.shell("tools: [Read, Glob]")),
                         ["Read", "Glob"])

    def test_a_shell_with_no_tools_line_answers_none(self):
        self.assertIsNone(orchestrator.parse_tools_line(self.shell("model: inherit")))

    def test_a_body_mentioning_tools_is_not_frontmatter(self):
        self.assertIsNone(orchestrator.parse_tools_line(
            "---\nname: x\n---\n\ntools: Read, Bash\n"))

    def test_an_overseer_is_read_only_only_when_it_names_no_writing_tool(self):
        self.assertTrue(orchestrator.is_read_only(["Read", "Grep", "Glob"]))
        self.assertFalse(orchestrator.is_read_only(["Read", "Bash"]))
        self.assertFalse(orchestrator.is_read_only(["Read", "Agent"]))
        self.assertFalse(orchestrator.is_read_only(None))
...
class ConfigValidation(unittest.TestCase):
    ...
    def test_the_shipped_shape_is_usable(self):
        self.assertEqual(orchestrator.validate_config(self.usable()), [])

    def test_a_foreign_schema_is_named(self):
        config = self.usable()
        config["schema"] = "inspire.emanate-config/0"
        self.assertEqual(len(orchestrator.validate_config(config)), 1)
        self.assertIn("schema", orchestrator.validate_config(config)[0])

    def test_the_four_required_keys_are_required(self):
        for key in ("schema", "tests_roots", "source_roots", "suite"):
            config = self.usable()
            del config[key]
            self.assertTrue(orchestrator.validate_config(config),
    ...
    def test_the_optional_keys_are_optional_and_still_typed(self):
        ...
        self.assertEqual(orchestrator.validate_config(config), [])
        config["checks"] = [{"roles": ["implementer"]}]
        self.assertTrue(orchestrator.validate_config(config))

    def test_a_missing_file_refuses_with_the_remedy(self):
        with tempfile.TemporaryDirectory() as root:
            with self.assertRaises(orchestrator.Refusal) as caught:
                orchestrator.load_config(os.path.join(root, "emanate.json"))
...
class StateWrites(unittest.TestCase):
    ...
    def test_the_state_round_trips(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            state = orchestrator.State(path, {"schema": orchestrator.STATE_SCHEMA,
                                              "units": {"auth.user": {"status": "pending"}}})
            state.save()
            state.unit("auth.user")["status"] = "promoted"
            state.save()
            reread = orchestrator.State.load(path)
    ...
    def test_a_write_leaves_no_partial_file_and_no_temporary_behind(self):
        with tempfile.TemporaryDirectory() as root:
            path = os.path.join(root, "state.json")
            orchestrator.write_json_atomic(path, {"a": 1})
            orchestrator.write_json_atomic(path, {"a": 2})
...
class Findings(unittest.TestCase):
    ...
    def test_a_finding_renders_the_three_slots(self):
        rendered = orchestrator.render_findings([orchestrator.finding(
    ...
    def test_a_brief_carries_the_findings_verbatim(self):
        brief = {"role": "tester", "unit_id": "auth.user",
                 "findings": [orchestrator.finding("citation-check", "CI-03", "stale", "copy it")]}
        rendered = orchestrator.render_brief(brief)
    ...
    def test_a_rule_finding_is_attributed_to_this_unit_by_path_or_id(self):
        ...
        self.assertTrue(orchestrator.targets_unit(unit["path"], unit))
        self.assertTrue(orchestrator.targets_unit("auth.user", unit))
        self.assertTrue(orchestrator.targets_unit("auth::user", unit))
        self.assertFalse(orchestrator.targets_unit("auth::org", unit))
        self.assertFalse(orchestrator.targets_unit("", unit))

    def test_jsonl_on_stderr_is_read_past_the_prose(self):
        rows = orchestrator.parse_jsonl(
...
class Versions(unittest.TestCase):
    ...
    def test_the_harness_version_is_read_out_of_whatever_it_prints(self):
        self.assertEqual(orchestrator.parse_version("2.1.259 (Claude Code)"), (2, 1, 259))
        self.assertGreaterEqual(orchestrator.parse_version("2.2.0"),
                                orchestrator.MIN_CLAUDE_VERSION)
        self.assertLess(orchestrator.parse_version("2.1.258"),
                        orchestrator.MIN_CLAUDE_VERSION)
        self.assertIsNone(orchestrator.parse_version("unknown"))
