...
Grounded in the scoping rule of [[adr-auth-01-identity-model]]. No
`references(…)` here on purpose: this fixture's graph is ED8's, and an edge to a
second entity would add a wave the worked example does not have.
...
| Action                                   | Touch | Notes             |
|------------------------------------------|-------|-------------------|
| [[auth.user.get\|auth::user::get]]       | read  | Reads one account. |
