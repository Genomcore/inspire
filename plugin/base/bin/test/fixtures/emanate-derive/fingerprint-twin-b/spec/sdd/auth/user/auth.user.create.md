...
## Preconditions
- `P1` — actor( admin ) — Only an administrator   provisions accounts directly.
- `P2` — absent(auth.user) — No account may already hold the submitted email.

## Behavior
3. `B1` —   Validate the email against the rules in [[auth-user-management]].
7. `B2` — Resolve  the organisation named by the input.
9. `B3` — Persist a new account row.
