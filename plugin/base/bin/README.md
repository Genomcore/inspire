...
| `emanate-harvest.sh` | **A tool, not a review rule.** Worktree diff → integration-branch commit (the emanation loop's harvest step, D4/D8): diffs a phase worktree against an integration branch since their common cut point, accepts only the phase's owned git pathspecs into one commit, refuses on conflict or an empty owned diff, and discards the worktree on request — sealing it onto a commit first, so plain `git worktree remove` takes it and no destructive git form is ever needed. `--autopsy <reason>` makes the one commit that is not a harvest: a stalled phase's whole tree, unfiltered, onto a branch that is never promoted. Pure git plumbing — the integration branch is never checked out, and `--mode plan`/`--dry-run` writes no ref, reflog entry or index. It emits no findings and is deliberately absent from `review.sh`'s `DEFAULT_RULES`. Needs `jq` (for its JSON summary) and does not source `_lib.sh`. | `/inspire-emanate run`'s harvest step, once per phase per unit, and its autopsy at a stall. |
