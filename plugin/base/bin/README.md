...
| `schemas/` | The **emanation plan's contract**, deployed to `.inspire/bin/schemas/` — read by consumers, never by the scripts. `emanation-plan.schema.json` is canonical (JSON Schema 2020-12, both documents `emanate-plan.sh` can print); the TypeScript and Python renderings live with their consumer, the INSPIRE factory. A stale schema is worse than none, so `test/test-plan-schema.sh` runs the real script and fails when a key the emitter prints is missing from it. Prose, exit codes and the `PR-*` catalogue stay in [`emanation-plan.md`](../skills/_references/emanation-plan.md): these files carry the shape, that file carries the meaning. | a viewer, an orchestrator, or anything else reading the plan's JSON. |
