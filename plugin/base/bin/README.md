...
| `emanate-orchestrator.py` | **A tool, not a review rule.** The unattended emanation loop itself, as a process (D8): `emanate-orchestrator.py run [--goal SEL] [--ceiling N] [--scope PATH]...` and `resume <run-id>`. It is the mechanics only — t=0's refusals, the wave loop, each unit's handoff sequence, arbitration, the drill, promotion, the report, and the state file a killed run resumes from; the judgment is the spawned agents'. Agents are reached through a **runner seam**, so the whole process runs without a model: `--runner fake:DIR` replays a script, which is how `plugin/test/orchestrator/` drives it. Python 3 — the one script here that is not bash — with its dependencies declared inline (PEP 723) and resolved by `uv run`, so a project carries no `pyproject` and no venv for it. Its mechanics live beside it in `lib/orchestrator/` — one folder per module of the loop, each holding the module and its own `test_<module>.py` (shared test stubs under `lib/orchestrator/test/`), imported through this file's own real path. Absent from `DEFAULT_RULES`. | `/inspire-emanate run`, as the session that loads the skill *is* the orchestrator; the process is spawned once per run. |
