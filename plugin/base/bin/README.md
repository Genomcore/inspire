...
| `lib/` | The sourced units the `emanate-*` scripts share, deployed to `.inspire/bin/lib/`: `derive-json.sh` (spools, table and keyed-entry readers, fingerprints, the jq prelude), `derive-types.sh` (the universal semantic vocabulary as data, and resolution against the project's own types), `derive-refusals.sh` (the rule sweep, the class map, the `DR-*` ids), `derive-domain.sh` and `derive-screen.sh` (one per unit kind). Sourceable on their own after `_lib.sh` + `_keyed-heads.sh`. | (libraries — not invoked directly) |
