...
| `../test/viewer-examples.py` | Builds cached plans from the three pinned `../reference-kbs` cases and starts the viewer in demo mode: `python3 plugin/test/viewer-examples.py serve`. The **Samples** button opens an overlay for switching between Simple case, Clinical recruitment portal, and Cross centre sharing console. `?example=clinical` and `?example=cross-centre` open a case directly. `prepare --force` refreshes cached plans after planner changes. Case working trees are not read or modified. | local viewer demonstrations using reference KBs. |
