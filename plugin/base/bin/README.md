...
| `emanate-plan-viewer.sh` · `viewer/` | **A local graph viewer for one plan document.** `emanate-plan-viewer.sh PLAN.json [--open] [--port N]` starts a loopback-only stdlib server and prints its URL. The HTML polls the selected JSON with caching disabled, so an atomic rewrite of the plan redraws its wave columns and dependency edges without a page reload. Ordering edges are solid, deferred references dashed, and targets absent from every wave remain visible as external or realized nodes — the same unknown-target path the plan contract requires every graph consumer to handle. The last valid graph stays visible if a partial write or invalid document is observed, with the source error reported inline until the next valid poll. | an operator watching a plan change while its scope or realization evidence evolves. |
