...
| `emanate-orchestrator.py` | **A tool, not a review rule.** The unattended emanation loop itself, as a process (D8): `emanate-orchestrator.py run [--goal SEL] [--ceiling N] [--scope PATH]...`, `resume <run-id>`, `check-citations --contract FILE`. It is the mechanics only — t=0's refusals, the wave loop, each unit's handoff sequence, arbitration, the drill, promotion, the report, and the state file a killed run resumes from; the judgment is the spawned agents'. Agents are reached through a **runner seam**, so the whole process runs without a model: `--runner fake:DIR` replays a script, which is how `plugin/test/orchestrator/` drives it. Python 3, standard library only, and the one script here that is not bash. Its mechanics live beside it in `lib/orchestrator/` — one module per phase of the loop, imported through this file's own real path — and its pure logic is covered by `test/test-orchestrator.py`. Absent from `DEFAULT_RULES`. | `/inspire-emanate run`, as the session that loads the skill *is* the orchestrator; the process is spawned once per run. |
