...
| `emanate-plan-viewer.sh` · `viewer/` | **A local graph viewer for one plan document.** `emanate-plan-viewer.sh PLAN.json [--open] [--port N]` starts a loopback-only stdlib server and prints its URL. The HTML polls the selected JSON with caching disabled, so an atomic rewrite redraws the plan without a page reload. The Waves view shows ordered columns; the Graph view places successive wave frontiers on concentric rings, groups directly linked units, aligns clusters with their dependencies, and supports fit, zoom, and drag-to-pan. Ordering edges are solid, deferred references dashed, and screen navigation is blue and dotted with a separate toggle alongside the other edge controls in the left pane; targets absent from every wave remain visible as external or realized nodes — the same unknown-target path the plan contract requires every graph consumer to handle. The last valid graph stays visible if a partial write or invalid document is observed, with the source error reported inline until the next valid poll. | an operator watching a plan change while its scope or realization evidence evolves. |
