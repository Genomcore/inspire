...
- `jq` (1.6+)
- `python3` (3.9+, only for the optional emanation-plan graph viewer)
...
| `emanate-gate.sh` | **A tool, not a review rule.** Claim coverage × citing tests × suite result → the **gate verdict** on stdout as JSON (D3/D4/D8): `emanate-gate.sh --contract FILE\|- --results FILE [--tests-root DIR]... [--previous FILE]` reads a derived contract's `claims[]`, greps the tests root(s) for `@claim` tokens (`tester.md`'s grammar), joins them against the `inspire.suite-results/1` manifest `emanate-results.sh` emits, and mints one `pass`/`fail` verdict plus the `GV-*` findings behind it. **Never runs a suite and never promotes** — the orchestrator does both; gate is the deterministic evidence an overseer's approval can never stand in for (D3). Writes nothing at all. Absent from `DEFAULT_RULES`. Shape, the suite-results schema, the exit codes and the `GV-*` classes: [`gate-verdict.md`](../skills/_references/gate-verdict.md). Needs `jq` and does not source `_lib.sh`. | `/inspire-emanate run`'s gate step, once per unit, before it **promotes git-side** — a merge carrying the verdict digest in a trailer, never a lifecycle edit (ED9). |
| `emanate-results.sh` | **A tool, not a review rule.** A test runner's own report → the `inspire.suite-results/1` manifest on stdout (D8): `emanate-results.sh --from FILE [--from FILE]... [--format jest] [--root DIR]`. **It defines no schema** — `lib/gate-results.sh` already reads that manifest, so this emits exactly what that file consumes and nothing it invents. `--from` repeats because the shipped stack runs unit and e2e as two commands; `--root` strips the runner's absolute paths (both the literal and the `pwd -P` spelling, since a `/var` → `/private/var` symlink would otherwise make the strip a silent no-op) so the `file` field joins against repo-relative `@claim` citations. An unrecognised assertion status **refuses** rather than folding into a bucket, and `--format` names `jest` as the one built dialect so an unbuilt one refuses loudly instead of being misread. Stdout is empty on every refusal, by construction. Absent from `DEFAULT_RULES`. | `/inspire-emanate run`'s verify step, once per unit, feeding `emanate-gate.sh --results`. |
| `schemas/` | The **emanation plan's contract**, deployed to `.inspire/bin/schemas/` — read by consumers, never by the scripts. `emanation-plan.schema.json` is canonical (JSON Schema 2020-12, both documents `emanate-plan.sh` can print); `emanation-plan.d.ts` and `emanation_plan.py` render the same shape natively, so a viewer and an orchestrator consume the plan without generating code at build time or taking a dependency — the Python is stdlib-only and typed for 3.9. Three renderings of one truth drift, and a stale one is worse than none, so `test/test-plan-schema.sh` runs the real script and fails when a key the emitter prints is missing from any of them. Prose, exit codes and the `PR-*` catalogue stay in [`emanation-plan.md`](../skills/_references/emanation-plan.md): these files carry the shape, that file carries the meaning. | a viewer, an orchestrator, or anything else reading the plan's JSON. |
| `emanate-run-state.py` · `schemas/emanation-run-state.*` · `schemas/emanation_run_state.py` | **Mutable execution state for one immutable plan snapshot.** `init PLAN STATE` creates pending entries for every wave unit; `update PLAN STATE` atomically records run and unit transitions. The JSON Schema is canonical; TypeScript and Python render the same contract, and the Python validator checks exact plan bytes and unit ids. The state records phase, attempts, delivery, stall and block outcomes; it is not realization evidence. Meaning and replan rules: [`emanation-run-state.md`](../skills/_references/emanation-run-state.md). | `/inspire-emanate run` and the optional viewer state overlay. |
| `emanate-plan-viewer.sh` · `viewer/` | **A local graph viewer for one plan document.** `emanate-plan-viewer.sh PLAN.json [--run-state STATE.json] [--open] [--port N]` starts a loopback-only stdlib server and prints its URL. The HTML polls the selected plan and optional run state with caching disabled. A matching run state adds progress counts, per-node state dots and phase details; a mismatched digest is reported without hiding the plan. The Waves view shows ordered columns; the Graph view places successive wave frontiers on concentric rings, groups directly linked units, aligns clusters with their dependencies, and supports fit, zoom, and drag-to-pan. Ordering edges are solid, deferred references dashed, and screen navigation is blue and dotted with a separate toggle alongside the other edge controls in the left pane; targets absent from every wave remain visible as external or realized nodes — the same unknown-target path the plan contract requires every graph consumer to handle. The last valid graph stays visible if a partial write or invalid document is observed, with the source error reported inline until the next valid poll. | an operator watching a plan and its build loop. |
| `viewer/demo.py` | Replays the complete `clean-three-waves` sample KB plan through delivery, verification stall, blocked dependent, retry, and completion. Run `python3 viewer/demo.py /tmp/inspire-demo-state.json`, then serve `test/fixtures/emanate-plan/clean-three-waves/expected-stdout.json` with `--run-state /tmp/inspire-demo-state.json`. `--once --step-seconds 0` produces a completed sidecar without waiting. This only simulates run-state updates; it executes no build agents. | viewer demonstrations. |
| `../test/viewer-examples.py` | Builds cached plans from the three pinned `../reference-kbs` cases and adds a bundled Genomics Suite plan with a simulated mid-run state. Start with `python3 plugin/test/viewer-examples.py serve`; the **Samples** overlay switches between all four cases. `?example=clinical`, `?example=cross-centre`, and `?example=genomics-suite` open a case directly. `prepare --force` refreshes cached reference plans after planner changes. Case working trees are not read or modified. | local viewer demonstrations using reference KBs and a build-loop snapshot. |
