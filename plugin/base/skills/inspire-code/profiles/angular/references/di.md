...
`inject()`-first dependency injection: provider scopes, injection tokens,
provider types, service patterns, hierarchical injection, cleanup with
`DestroyRef`, and testing with DI.
...
Collect multiple values for the same token:
...
Run async code before the app starts using `provideAppInitializer`:
...
Run code with an injection context:
...
Use abstract classes for better type safety than interface + string token:
...
### takeUntilDestroyed outside the constructor
