...
Data fetching with the traditional `HttpClient` and, on recent Angular
versions, the signal-based `resource()` and `httpResource()`. Detect the
Angular version in use from `@angular/core` in `package.json` before
recommending version-gated APIs.

## httpResource() — signal-based HTTP

`httpResource()` was introduced in Angular 19.2 (developer preview) and is
stable since Angular 22. In v19–v21 the API may differ — verify for the
...
For complex scenarios or when Observable operators are needed:
