...
- **Which-side probe.** "That rule is about another entity's rows — is this the entity that can hold it? Referential integrity is declared where the reference is: `references(auth.org)` on the referring field, not as an invariant on the entity being referred to. Written on this side it asserts something this entity emits nothing to enforce." Use whenever an invariant names another entity, and always when it reads *"every X that points here resolves"*.
