...
- **Inheritance of errors.** `signup` lists `(Inherits errors from [[auth.user.create|auth::user::create]])` rather than restating the error catalog. The agent expands this when rendering with `/inspire-domain show`.
