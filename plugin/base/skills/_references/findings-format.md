...
Conversational skills that report SDD-layer issues — `/inspire-domain review`, `/inspire-module scan`, `/inspire-feature scan`, source/show traces — use a single rendering format so operators can read every report the same way.
...
**Suggested follow-up.** Reconcile the type. Either rewrite the read action to expect `uuid`, or change create to write `integer`. Re-run `/inspire-domain review auth::user` after.
```
