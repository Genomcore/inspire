...
- `accepted → stable` — run stable-blockers + touched-entity-lifecycle; refuse if any `requires` target is not yet stable, or any touched entity is below `accepted`. Entities promote independently. The OMP Ralph loop does not change KB lifecycle: its worktree merges are code-side only.
