...
Conversational skills that report SDD-layer issues — `/openbims_object review`, `/openbims_module scan`, `/openbims_feature scan`, source/show traces — use a single rendering format so operators can read every report the same way.
...
**Suggested follow-up.** Reconcile the type. Either rewrite the read action to expect `uuid`, or change create to write `integer`. Re-run `/openbims_object review auth::user` after.
```
...
| `body contains TODO marker` / `FIXME` / `XXX` / `HACK` | no-todos | Outstanding-work marker in body. Move to `tracker/tickets/`. |
